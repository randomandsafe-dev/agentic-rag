...
在 `.env` 配置 `TAVILY_API_KEY` 后重启应用。Web UI 会并列运行本地检索和联网搜索：两类资料分别发送给 AI、生成两份独立回答，并各自展示对应的原始来源，避免混合上下文。

联网路由支持 `auto`、`local_only`、`web_only` 和 `both`。自动模式默认只查询本地；检测到”今天、最新、实时、新闻、价格”等时效性需求时使用联网搜索；同时包含内部资料与实时需求时双路查询。包含密码、Token、身份证等敏感词的请求会强制留在本地。

联网结果会先按搜索相关度、域名可信度与发布时间进行评分，再通过可用的 Cross-Encoder 重排序到 Top-K。每份本地或联网回答生成后都会基于各自的证据运行答案验证，并在页面显示可信度、潜在幻觉和信息遗漏提示。
